const header = {
  homepage: '/',
  title: 'Jidoggs',
}

const about = {
  name: 'Olajide Shonuga',
  role: 'Front End Engineer',
  description:
    'I love bringing Ideas to life with the help of web technologies.',
  resume: 'https://drive.google.com/file/d/1facnIMBKcQJhZB8O2jh2JsT498H8Xf1M/view?usp=sharing',
  social: {
    linkedin: 'https://linkedin.com/in/jide-shonuga',
    github: 'https://github.com/jidoggs',
  },
}

const projects = [
  {
    name: 'Food Ordering',
    description:
      'A food ordering Application where you can add and remove food itemms to cart.',
    stack: ['SASS', 'JavaScript', 'React'],
    sourceCode: 'https://github.com/jidoggs/stutern-food-delivery-app',
    livePreview: 'http://stutern-food-delivery-app.vercel.app/',
  },
  {
    name: 'Traveler',
    description:
      'A Travel application where a user can search for travel destination, get weather report of the said destination, travel advice about the said destinantion and others.',
    stack: ['SASS', 'JavaScript', 'React'],
    sourceCode: 'https://github.com/jidoggs/traveler',
    livePreview: 'https://traveler-env-jidoggs.vercel.app/',
  },
  {
    name: 'Nba Clone',
    description:
      'This is a Mobile only Nba clone that gives users access to Nba team news, Games Highlights, Team Data, Nba standing.',
    stack: ['SASS', 'JavaScript', 'React'],
    sourceCode: 'https://github.com/jidoggs/NBA-clone',
    livePreview: 'https://nba-clone.vercel.app/',
  },
  {
    name: 'Box Office Mojo Clone',
    description:
      'A static clone of the popular box office mojo landing page built using 90% HTMl tables.',
    stack: ['CSS', 'HTML'],
    sourceCode: 'https://github.com/jidoggs/box-mojo',
    livePreview: 'https://jidoggs.github.io/box-mojo/',
  },
  {
    name: 'Hacker News',
    description:
      'A static clone of the popular Hacker news landing page.',
    stack: ['CSS', 'HTML'],
    sourceCode: 'https://github.com/jidoggs/hacker-news',
    livePreview: 'https://jidoggs.github.io/hacker-news/',
  },
  {
    name: 'Trafico',
    description:
      'A multi section landing page for a traffic management conultancy company.',
    stack: ['CSS','JavaScript', 'HTML'],
    sourceCode: 'https://github.com/jidoggs/trafigo',
    livePreview: 'https://jidoggs.github.io/trafigo/',
  },
]

const skills = [
  'HTML',
  'CSS',
  'JavaScript',
  'TypeScript',
  'React',
  'Redux',
  'SASS',
  'ANTD',
  'Material UI',
  'Git',
]

const contact = {
  email: 'tomiwa.shonuga@gmail.com',
}

export { header, about, projects, skills, contact }
